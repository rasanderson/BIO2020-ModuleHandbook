...
I am in the process of preparing some short (5 to 10 minutes) animated videos to explain in simple, non-technical terms some quantitative biology concepts. These are new for 2021-22 and hopefully you will find them useful.

### Section Zero on Canvas
The Canvas module headed **Section 0: Getting ready for BIO2020** contains advice on installing R/RStudio, if using your own laptop/MacBook, as well as a quick-reference sheet for the module (CheatSheet). The latter will probably be incomprehensible at the start of this course, but should soon be a valuable resource to help you.
...
The R software is pre-installed on University cluster-room and Library PCs. It can also be used on your own PC (it is compatible with Windows, Mac OS X and Linux), or you can use the Newcastle University Windows Virtual Desktop (WVD) which simply requires access to a browser or the WVD client <https://services.ncl.ac.uk/itservice/core-services/software/wvd/#accesswvd>. If you have a Windows-based tablet then R will run on that (it will not run on an Android tablet).

We will assume that most students will do the practicals when timetabled in the main cluster rooms. **If you want to bring along your own laptop or MacBook to these computer cluster practicals feel free to do so**. You can download the R software from <https://cran.r-project.org/> by selecting your operating system and downloading the latest release.

The two main ways of running R software are through the default client, or a more complex and user-friend system called RStudio Desktop. **We recommend you use RStudio Desktop**. This is pre-installed in the Campus PC clusters. If you want to install it on your own laptop or MacBook, then bring it along to the first practical and we will help you. 
